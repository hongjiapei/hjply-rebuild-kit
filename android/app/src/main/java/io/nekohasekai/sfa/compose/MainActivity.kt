...
class MainActivity :
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // hjply is distributed privately. Do not contact upstream release
        // channels or show their update prompts.
        ...
        setContent {
            // Hoist the dashboard VM to the root so SFATheme can drive its
            // darkTheme flag from Settings.themeDarkMode, and so the entire
            // tree below can read the active palette via LocalHjplyPalette.
            // The SFAApp() body calls viewModel() again — both calls return
            // the same activity-scoped instance.
            val dashboardViewModel: DashboardViewModel = viewModel()
            if (!::dashboardViewModel.isInitialized) {
                this.dashboardViewModel = dashboardViewModel
            }
            val dashboardUiState by dashboardViewModel.uiState.collectAsState()
            val isDark = dashboardUiState.isDarkMode

            SFATheme(darkTheme = isDark) {
                CompositionLocalProvider(
                    LocalHjplyPalette provides if (isDark) DarkHjplyPalette else LightHjplyPalette,
                ) {
                    SFAApp()
                }
            }
    ...
    fun SFAApp() {
        ...
        // Upstream update discovery is intentionally disabled for hjply.
        ...
        val railScreens = listOf(Screen.Dashboard)
        ...
        val scaffoldContent: @Composable (PaddingValues) -> Unit = { paddingValues ->
            Box(
                ...
                if (useNavigationRail) {
                    ...
                    // Start FAB disabled: the dashboard already provides a central connect button,
                    // and the floating play button does not match the target design prototype.
                    androidx.compose.animation.AnimatedVisibility(
                        visible = false,
                        enter = scaleIn(),
                        exit = scaleOut(),
